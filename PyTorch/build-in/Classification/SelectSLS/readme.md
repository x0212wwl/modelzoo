...
SelectSLS 是一种新的构建卷积神经网络模块的方法，通过在一个残差块内构建分层的残差类连接，以更细粒度的级别表示多尺度特征，从而提高了各类视觉任务的性能。
...
```bash
python weloTrain.py --name train --arch SelectSLS --dataset cifar100 --datapath ../data --batch_size 128 --epochs 100
...
* `--name`: 任务名称 (train)
* `--arch`: 模型架构名称 (res2net)
* `--dataset`: 使用的数据集 (cifar100)
* `--datapath`: 数据集路径 (默认为 ../data)
* `--batch_size`: 批大小 (128)
* `--epochs`: 训练轮数 (100)
